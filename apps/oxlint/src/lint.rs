...
impl CliRunner {
    ...
    pub fn run(self, stdout: &mut dyn Write) -> CliRunResult {
        ...
        let debug_timings = self.options.output_options.debug_timings;
        ...
        let lint_result = if let Some(rule_timing_store) = &rule_timing_store {
            lint_runner.lint_files_with_rule_timings(
                &files_to_lint,
                tx_error.clone(),
                &diff_manager,
                rule_timing_store,
            ...
            lint_runner.lint_files(&files_to_lint, tx_error.clone(), &diff_manager)
        };
