...
fn format_rule_timing_table(rule_timings: &[RuleTimingRecord]) -> String {
    ...
    let calls_width = rule_timings
        .iter()
        .filter_map(|record| record.calls)
        .map(|calls| calls.to_string().len())
        .max()
        .unwrap_or("Calls".len())
        .max("Calls".len());
    let total_millis =
    ...
    for (record, rule_name) in rule_timings.iter().zip(rule_names) {
        ...
        let calls = record.calls.map_or_else(|| "-".to_string(), |calls| calls.to_string());
        writeln!(
            output,
...
mod test {
    ...
    #[test]
    fn lint_command_info_with_rule_timings() {
        let formatter = DefaultOutputFormatter;
        let result = formatter.lint_command_info(&LintCommandInfo {
            number_of_files: 1,
